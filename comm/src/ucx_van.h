  ...
  void CloseEp(ucp_ep_h ep) {
    ...
    if (UCS_PTR_IS_PTR(req)) {
      ...
      if ((status != UCS_OK) && (status != UCS_ERR_ENDPOINT_TIMEOUT)) {
        PS_LOG_ERROR << "failed to close ep: " << ep << "("
...
public:
  ...
  void Init(Node *my_node, UCXVan *van) {
    ...
    // - Recv worker is created with multi-thread support, because it is created
    //   in the initialization thread but accessed/used by the UCX Van RX polling thread.
    //   UCX requires multi-thread mode when worker is accessed from different threads.
    tx_worker_ = CreateWorker(true);
    rx_worker_ = CreateWorker(true);
...
private:
  void SetGpuDeviceId() {
    ...
    if (src_dev_type_ == GPU) {
      ...
      if (cudaSuccess != cerr) {
        PS_LOG_ERROR << "failed to set device " << src_dev_idx_ << ": " << cerr;
      }
    }
#else
    ...
    if (queue_sends_) {
      ...
      if (UCS_PTR_IS_ERR(st)) {
        ...
        if (Van::err_handle_ != nullptr) {
          ...
          PS_LOG_ERROR << "failed to send data: " << ucs_status_string(st_val)
                     << ". " << msg.DebugString();
        }
        return -1;
      }
      ...
      if (!UCS_PTR_IS_PTR(st)) {
        ...
        if (UCS_PTR_IS_ERR(st)) {
          if (Van::err_handle_ != nullptr) {
            ...
            PS_LOG_ERROR << "failed to send meta data: " << ucs_status_string(UCS_PTR_STATUS(st))
                       << ". " << msg.DebugString();
          }
          return -1;
        }
    ...
    while (!should_stop_.load()) {
      ...
      for (int i = 0; i < num ; ++i) {
        ...
        if (!UCS_PTR_IS_PTR(st)) {
            ...
            if (UCS_PTR_IS_ERR(st)) {
                PS_LOG_ERROR << "failed to send data: " << ucs_status_string(UCS_PTR_STATUS(st));
            }
        }
      }
