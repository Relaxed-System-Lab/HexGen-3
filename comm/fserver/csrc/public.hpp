...
// Debug function to get key information for a handler
std::tuple<std::vector<uint64_t>, std::vector<uint64_t>, int, int> debug_get_handler_keys(uint64_t handler) {
  std::vector<uint64_t> push_keys;
  std::vector<uint64_t> pull_keys;
  int sender_rank = -1;
  int single = -1;
  ...
  {
    std::lock_guard<std::mutex> lock(mu_);
    auto it = meta_map_.find(handler);
    if (it != meta_map_.end()) {
      const AFTensorMeta& reqmeta = it->second;
      sender_rank = reqmeta.sender_rank;
      single = reqmeta.single ? 1 : 0;
      
      for (const auto& t : reqmeta.push_tensors) {
        push_keys.push_back(t.key);
      }
      
      for (const auto& t : reqmeta.pull_tensors) {
        pull_keys.push_back(t.key);
      }
    }
  }
  
  return std::make_tuple(push_keys, pull_keys, sender_rank, single);
}
...
void pybind_public(py::module &m){
  ...
  // Debug function
  m.def("debug_get_handler_keys", &debug_get_handler_keys, 
        py::arg("handler"),
        py::call_guard<py::none>());
}
