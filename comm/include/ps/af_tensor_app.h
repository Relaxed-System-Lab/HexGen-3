...
class AFTensorServer {
 public:
   ...
   *
   * @param instance_idx the instance index within the group (for Postoffice routing)
   * Note: The actual GPU device ID is read from STEPMESH_GPU environment variable,
   * not from this parameter. This allows instance_idx=0 (for rank calculation)
   * while using different GPU devices (0, 1, 2, 3, etc.)
   */
  explicit AFTensorServer(int instance_idx)
      : kv_(0, false, instance_idx), response_stop_(false), instance_idx_(instance_idx) {
    gpu_ = -1;
    Environment::Get()->find("STEPMESH_GPU", &gpu_, gpu_);
    PS_CHECK_GE(gpu_, 0) << "STEPMESH_GPU is not set";
    PS_LOG(INFO) << "AFTensorServer instance_idx=" << instance_idx << " runs on gpu " << gpu_;
    Backend::Get()->SetDevice(gpu_);
    for (int i = 0; i < 64; i++) {
      events_.push_back(new TensorEvent());
    }
 ...
 private:
  ...
  void ResponseWorker() {
    ...
    while (!response_stop_.load()) {
      ...
      if (rsp.event != nullptr) {
        // Add timeout check for event sync
        auto sync_start = std::chrono::steady_clock::now();
        rsp.event->Sync();
        auto sync_end = std::chrono::steady_clock::now();
        auto sync_duration = std::chrono::duration_cast<std::chrono::milliseconds>(sync_end - sync_start).count();
        rsp.event->Release();
        rsp.event = nullptr;
      }
  ...
  void KVHandler(const KVMeta& req_meta, const KVPairs<char>& req_data) {
    ...
    if (req_meta.cmd == 0) {
      ...
      af_meta.single = true;
      af_meta.sender_rank =
          Postoffice::GetServer(instance_idx_)->IDtoRank(af_meta.sender);
  ...
  int gpu_;
  /** \brief instance id in one group (for Postoffice routing) */
  int instance_idx_;
