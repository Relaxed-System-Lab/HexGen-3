...
class ModelRunner:
    ...
    def forward_decode(
        ...
        if not skip_attn_backend_init:
            with nvtx_range("mr.decode.attn_init"):
                self.attn_backend.init_forward_metadata(forward_batch)
        # FIXME: add pp_proxy_tensors arg to all models
        kwargs = {}
        if self.support_pp:
            kwargs["pp_proxy_tensors"] = pp_proxy_tensors
        with nvtx_range("mr.decode.model_forward"):
    ...
    def forward_extend(
        ...
        if not skip_attn_backend_init:
            with nvtx_range("mr.extend.attn_init"):
                self.attn_backend.init_forward_metadata(forward_batch)
        ...
        with nvtx_range("mr.extend.model_forward"):
            return self.model.forward(
                forward_batch.input_ids,
                forward_batch.positions,
                forward_batch,
    ...
    def forward(
        ...
        with get_global_expert_distribution_recorder().with_forward_pass(
            ...
            nvtx_name = f"mr.forward.{forward_batch.forward_mode.name}"
    ...
    def _forward_raw(
        ...
        if forward_batch.forward_mode.is_decode():
            with nvtx_range("mr.forward.decode"):
            ...
            with nvtx_range("mr.forward.extend"):
