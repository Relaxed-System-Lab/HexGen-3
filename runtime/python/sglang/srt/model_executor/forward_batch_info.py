...
class ForwardBatch:
    ...
    def init_new(
        ...
        # FFN: create minimal ForwardBatch only, skip most unnecessary setup
        if afd_is_ffn():
            device = model_runner.device
            batch_size = len(batch.seq_lens) if batch.seq_lens is not None else 0

            if batch.seq_lens is None:
                seq_lens = torch.zeros(batch_size, dtype=torch.int64, device=device)
                seq_lens_sum = 0
            else:
                seq_lens = batch.seq_lens
                seq_lens_sum = batch.seq_lens_sum if batch.seq_lens_sum is not None else seq_lens.sum().item()
            
            if batch.input_ids is None:
                # For dummy batch: input_ids shape [num_tokens] with num_tokens = batch_size
            ...
            ret = cls(
                ...
                input_ids=input_ids,  # used for shape check in postprocess_layer
                req_pool_indices=None,  # FFN does not need
                seq_lens=seq_lens,  # for batch_size and seq_lens_sum
                ...
                seq_lens_sum=seq_lens_sum,  # for logging
                ...
                lora_paths=None,
                sampling_info=batch.sampling_info,  # may be needed, pass through from batch
            ...
            # FFN only sets positions (for postprocess_layer); rest skipped
            if ret.forward_mode.is_decode():
                ret.positions = torch.arange(batch_size, dtype=torch.int64, device=device)
            else:
                ret.positions = torch.zeros(batch_size, dtype=torch.int64, device=device)  # prefill: 1 token per request

            ret.afd_split_seq_index = batch.tbo_split_seq_index  # then AfdForwardBatchPreparer.prepare builds afd_children
            import logging
