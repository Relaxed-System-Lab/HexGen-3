...
class SchedulerOutputProcessorMixin:
    ...
    def process_batch_result_prefill(
        ...
        # process is not needed for FFN in AFD mode
    ...
    def update_metrics_prefill(
        self:Scheduler,
        batch: ScheduleBatch,
    ):
        """Update AFD metrics counters for prefill phase."""
        from sglang.srt.layers.afd import afd_is_attn, afd_is_ffn, get_afd_perspective
        
        # Count prefill tokens (extend_input_len for each request)
        num_prefill_tokens = 0
        for req in batch.reqs:
            if hasattr(req, 'extend_input_len'):
                num_prefill_tokens += req.extend_input_len

        # logger.info(f"Updating prefill metrics: num_prefill_tokens={num_prefill_tokens}")

        if get_afd_perspective() != None:
            if hasattr(self, 'prefill_tokens_total'):
                self.prefill_tokens_total += num_prefill_tokens
    ...
    def update_metrics_decode(
        self: Scheduler,
        batch: ScheduleBatch,
    ):
        """Update AFD metrics counters for decode phase."""
        from sglang.srt.layers.afd import afd_is_attn, afd_is_ffn, get_afd_perspective
        ...
        # AFD mode: count tokens based on worker type
        # For decode phase: each request generates 1 new token, so use batch_size

        num_tokens = len(batch.reqs)  # batch_size

        if afd_is_attn():
            if hasattr(self, 'attn_tokens_total'):
                self.attn_tokens_total += num_tokens
        elif afd_is_ffn():
            if hasattr(self, 'ffn_tokens_total'):
                self.ffn_tokens_total += num_tokens
    ...
    def stream_output_generation(
        ...
        for req in reqs:
            ...
            if req.finished():
                ...
                self._update_autoscaling_finished_counters(req)
                should_output = True
            else:
                if req.stream:
                    stream_interval = (
