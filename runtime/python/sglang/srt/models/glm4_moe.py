...
class Glm4MoeMLP(nn.Module):
    ...
    def forward(self, x, forward_batch=None, can_fuse_mlp_allreduce=False):
        with nvtx_range("glm4_mlp.forward"):
            if (self.tp_size == 1) and x.shape[0] == 0:
                return x

            with nvtx_range("glm4_mlp.gate_up_proj"):
                gate_up, _ = self.gate_up_proj(x)
            with nvtx_range("glm4_mlp.act"):
                x = self.act_fn(gate_up)
            with nvtx_range("glm4_mlp.down_proj"):
                x, _ = self.down_proj(
                    x, can_fuse_mlp_allreduce=can_fuse_mlp_allreduce
                )
            return x
...
class Glm4MoeDecoderLayer(DeepseekV2DecoderLayer):
    def __init__(
        ...
        # AFD: depending on the current perspective, we either execute the real
        # attention/ffn or replace it with a lightweight proxy.
...
class Glm4MoeModel(DeepseekV2Model):
    ...
    def forward(
        ...
        if not forward_batch.can_run_afd_overlap:
...
class Glm4MoeForCausalLM(DeepseekV2ForCausalLM):
    ...
    def forward(
        ...
        # AFD FFN-side only: skip logits computation.
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]], is_nextn=False):
        ...
        if afd_perspective == AFDPerspective.AFD_PERSPECTIVE_FFN:
            # Attention is replaced by AFDProxyAttention -> q/k/v weights are unused.
            stacked_params_mapping = [
                ("gate_up_proj", "gate_proj", 0),
                ("gate_up_proj", "up_proj", 1),
            ]
        elif afd_perspective == AFDPerspective.AFD_PERSPECTIVE_ATTN:
            # MLP is replaced by AFDProxyMLP -> (fused) gate/up weights are unused.
            stacked_params_mapping = [
                ("qkv_proj", "q_proj", "q"),
                ("qkv_proj", "k_proj", "k"),
                ("qkv_proj", "v_proj", "v"),
            ...
            stacked_params_mapping = stacked_params_mapping_all
