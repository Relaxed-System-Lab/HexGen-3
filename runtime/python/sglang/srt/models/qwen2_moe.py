...
class Qwen2MoeForCausalLM(nn.Module):
    ...
    def forward(
        ...
        # In AFD mode, FFN only returns hidden_states; logits (sampling, output, etc.) are handled by Attn.
        from sglang.srt.layers.afd import afd_is_ffn
        if afd_is_ffn():
            return hidden_states  # Sent back to Attn via StepMesh
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                for mapping in expert_params_mapping:
                    ...
                    # Skip loading MLP weights in ATTN perspective and attention weights in FFN perspective
                    if afd_perspective == AFDPerspective.AFD_PERSPECTIVE_ATTN and "mlp" in name:
                        continue
                    if afd_perspective == AFDPerspective.AFD_PERSPECTIVE_FFN and "self_attn" in name:
                        continue
