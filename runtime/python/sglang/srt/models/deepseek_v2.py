...
class DeepseekV2MoE(nn.Module):
    ...
    def forward(
        ...
        with nvtx_range(f"layer{self.layer_id}.moe.forward"):
    ...
    def forward_normal_dual_stream(
        ...
        with nvtx_range(f"layer{self.layer_id}.moe.forward_normal_dual_stream"):
            current_stream = torch.cuda.current_stream()
            self.alt_stream.wait_stream(current_stream)
            with nvtx_range(f"layer{self.layer_id}.moe.shared_experts"):
                shared_output = self._forward_shared_experts(hidden_states)

            with torch.cuda.stream(self.alt_stream):
                with nvtx_range(f"layer{self.layer_id}.moe.router_gate"):
                    router_logits = self.gate(hidden_states)
                kwargs = {"hidden_states": hidden_states}
                if self.topk is not None:
                    with nvtx_range(f"layer{self.layer_id}.moe.topk"):
                        kwargs["topk_output"] = self.topk(hidden_states, router_logits)
                else:
                    kwargs["router_logits"] = router_logits
                with nvtx_range(f"layer{self.layer_id}.moe.experts"):
                    final_hidden_states = self.experts(**kwargs)
                if not _is_cuda:
                    final_hidden_states *= self.routed_scaling_factor
            current_stream.wait_stream(self.alt_stream)
            with nvtx_range(f"layer{self.layer_id}.moe.add_shared"):
                final_hidden_states += shared_output
            if self.tp_size > 1 and not can_fuse_mlp_allreduce:
                with nvtx_range(f"layer{self.layer_id}.moe.tp_all_reduce"):
    ...
    def forward_normal(
        ...
        with nvtx_range(f"layer{self.layer_id}.moe.forward_normal"):
