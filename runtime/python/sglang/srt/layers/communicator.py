...
class CommunicateSummableTensorPairFn:
    ...
    def _gather(
        ...
        if torch.cuda.is_available():
            ...
            hidden_size_mb = hidden_states.numel() * hidden_states.element_size() / 1024**2
            residual_size_mb = residual.numel() * residual.element_size() / 1024**2 if residual is not None else 0
            gathered_buffer_size_mb = (
                forward_batch.gathered_buffer.numel() * forward_batch.gathered_buffer.element_size() / 1024**2
                if forward_batch.gathered_buffer is not None else 0
            )
            logger.info(
                f"[CommunicateSummableTensorPairFn Memory] _gather: START, "
                f"hidden_states.shape={hidden_states.shape}, hidden_size={hidden_size_mb:.2f}MB, "
                f"residual_size={residual_size_mb:.2f}MB, gathered_buffer_size={gathered_buffer_size_mb:.2f}MB, "
                f"allocated={mem_before:.2f}GB"
            )
        ...
        # memory monitoring after gather
        if torch.cuda.is_available():
            mem_after = torch.cuda.memory_allocated() / 1024**3
            mem_reserved_after = torch.cuda.memory_reserved() / 1024**3
            logger.info(
                f"[CommunicateSummableTensorPairFn Memory] _gather: DONE, "
                f"hidden_states.shape={hidden_states.shape}, "
                f"Δallocated={mem_after-mem_before:.2f}GB, "
                f"Δreserved={mem_reserved_after-mem_reserved_before:.2f}GB"
            )
    ...
    def _scatter(
        ...
        if torch.cuda.is_available():
            ...
            hidden_size_mb = hidden_states.numel() * hidden_states.element_size() / 1024**2
            logger.info(
                f"[CommunicateSummableTensorPairFn Memory] _scatter: START, "
                f"hidden_states.shape={hidden_states.shape}, hidden_size={hidden_size_mb:.2f}MB, "
                f"attn_tp_size={context.attn_tp_size}, allocated={mem_before:.2f}GB"
            )
        ...
        # memory monitoring after scatter
        if torch.cuda.is_available():
            mem_after = torch.cuda.memory_allocated() / 1024**3
            mem_reserved_after = torch.cuda.memory_reserved() / 1024**3
            logger.info(
                f"[CommunicateSummableTensorPairFn Memory] _scatter: DONE, "
                f"hidden_states.shape={hidden_states.shape}, "
                f"Δallocated={mem_after-mem_before:.2f}GB, "
                f"Δreserved={mem_reserved_after-mem_reserved_before:.2f}GB, "
                f"tensor_list_len={len(tensor_list)}"
            )
