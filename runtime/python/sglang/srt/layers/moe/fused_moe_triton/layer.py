...
class FusedMoE(torch.nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, topk_output: TopKOutput):
        with nvtx_range(f"FusedMoE.forward[layer={self.layer_id}]"):
            assert self.quant_method is not None


            # Matrix multiply.
            with nvtx_range(f"FusedMoE.forward.quant_apply[layer={self.layer_id}]"):
            ...
            if self.reduce_results and (self.tp_size > 1 or self.ep_size > 1):
                with nvtx_range(f"FusedMoE.forward.all_reduce[layer={self.layer_id}]"):
