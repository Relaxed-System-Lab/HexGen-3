...
class EPMoE(FusedMoE):
    ...
    def forward(self, hidden_states: torch.Tensor, topk_output: TopKOutput):
        with nvtx_range(f"EPMoE.forward[layer={self.layer_id}]"):
    ...
    def forward_deepgemm(
        ...
        with nvtx_range(f"EPMoE.forward_deepgemm[layer={self.layer_id}]"):
            self.w13_weight_fp8 = (
                self.w13_weight,
                (
            ...
            with nvtx_range(f"EPMoE.deepgemm.prepare_scales[layer={self.layer_id}]"):
            ...
            # PreReorder
            with nvtx_range(f"EPMoE.deepgemm.pre_reorder[layer={self.layer_id}]"):
            ...
            # GroupGemm-0
            with nvtx_range(f"EPMoE.deepgemm.gemm0[layer={self.layer_id}]"):
                gateup_input_fp8 = (
                    gateup_input,
            ...
            # Act
            with nvtx_range(f"EPMoE.deepgemm.activation[layer={self.layer_id}]"):
            ...
            # GroupGemm-1
            with nvtx_range(f"EPMoE.deepgemm.gemm1[layer={self.layer_id}]"):
            ...
            # PostReorder
            with nvtx_range(f"EPMoE.deepgemm.post_reorder[layer={self.layer_id}]"):
                output = torch.empty(
    ...
    def forward_normal(self, hidden_states: torch.Tensor, topk_output: TopKOutput):
        with nvtx_range(f"EPMoE.forward_normal[layer={self.layer_id}]"):
    ...
    def run_moe(self, hidden_states: torch.Tensor, topk_output: TopKOutput):
        with nvtx_range(f"EPMoE.run_moe[layer={self.layer_id}]"):
            topk_weights, topk_ids, _ = topk_output
            ...
            with nvtx_range(f"EPMoE.run_moe.preprocess[layer={self.layer_id}]"):
            ...
            # PreReorder
            with nvtx_range(f"EPMoE.run_moe.pre_reorder[layer={self.layer_id}]"):
            ...
            # GroupGemm-0
            with nvtx_range(f"EPMoE.run_moe.gemm0[layer={self.layer_id}]"):
            ...
            # Act
            with nvtx_range(f"EPMoE.run_moe.activation[layer={self.layer_id}]"):
                if self.activation_scheme == "dynamic" and not self.use_block_quant:
                    self.w2_input_scale = None
                    down_input = torch.empty(
                        gateup_output.shape[0],
            ...
            # GroupGemm-1
            with nvtx_range(f"EPMoE.run_moe.gemm1[layer={self.layer_id}]"):
                down_output = torch.empty(
                    down_input.shape[0],
            ...
            # PostReorder
            with nvtx_range(f"EPMoE.run_moe.post_reorder[layer={self.layer_id}]"):
                output = torch.empty(
                    hidden_states_shape, dtype=hidden_states_dtype, device=hidden_states_device
                )
                post_reorder_triton_kernel[(hidden_states_shape[0],)](
