...
class AfdForwardBatchPreparer:
    ...
    def prepare(cls, batch: ForwardBatch):
        ...
        # FFN simplified: create simple afd_children directly, no complex filter_batch
        if afd_is_ffn():
            cls.prepare_ffn_simplified(batch)
        else:
            # Attn full path: use full prepare_raw
    ...
    @classmethod
    def prepare_ffn_simplified(cls, batch: ForwardBatch):
        """FFN simplified: create simple afd_children with only necessary fields.

        FFN mode does not need:
        - Complex token computation (out_num_token_non_padded)
        - Real input_ids/positions slicing (FFN receives hidden_states from Attn)
        - req_pool_indices, out_cache_loc, etc. (FFN does not manage KV cache)

        Only needs:
        - afd_children list (for micro-batch iteration)
        - afd_parent_token_range (for hidden_states slicing)
        """
    ...
    def _create_simple_ffn_child(
        ...
        """Create a simple child ForwardBatch for FFN with only necessary fields."""
        child_batch_size = end_seq_index - start_seq_index

        # Simple seq_lens (1 token per sequence)
        child_seq_lens = torch.ones(child_batch_size, dtype=torch.int64, device=batch.seq_lens.device)
        
        # Dummy input_ids (FFN does not need real input_ids)
        child_input_ids = torch.zeros(end_token_index - start_token_index, dtype=torch.int64, device=batch.input_ids.device)
        
        # Dummy positions (FFN does not need real positions)
        child_positions = torch.zeros(end_token_index - start_token_index, dtype=torch.int64, device=batch.positions.device)
        
        return ForwardBatch(
            forward_mode=batch.forward_mode,
            batch_size=child_batch_size,
            input_ids=child_input_ids,
            req_pool_indices=None,  # FFN does not need
            seq_lens=child_seq_lens,
            out_cache_loc=None,  # FFN does not need
            seq_lens_sum=child_batch_size,
            seq_lens_cpu=None,
            return_logprob=False,
            top_logprobs_nums=None,
            token_ids_logprobs=None,
            positions=child_positions,
            extend_num_tokens=end_token_index - start_token_index,
            extend_seq_lens=None,
            ...
            lora_paths=None,
            sampling_info=None,  # FFN does not need
            ...
            afd_parent_token_range=(start_token_index, end_token_index),  # Key: used for hidden_states slicing
            afd_children=None,
            can_run_afd_overlap=None,
            is_extend_in_batch=False,
            can_run_dp_cuda_graph=False,
            global_forward_mode=None,
            num_token_non_padded=torch.tensor([end_token_index - start_token_index], dtype=torch.int32, device=batch.input_ids.device),
        )
    ...
    def _compute_split_token_index(cls, batch: ForwardBatch):
        ...
        # FFN: split by split_seq_index and token_num_per_seq (no real extend_seq_lens needed)
        if afd_is_ffn():
            if m == 2:
                split_seq_index = batch.afd_split_seq_index[0]
                # decode: 1 token/seq; prefill: same assumption for FFN dummy batch
                tokens_per_seq = token_num_per_seq if token_num_per_seq is not None else 1
                return [split_seq_index * tokens_per_seq]
            elif m == 3:
                split_seq_index = batch.afd_split_seq_index
                tokens_per_seq = token_num_per_seq if token_num_per_seq is not None else 1
                return [
                    split_seq_index[0] * tokens_per_seq,
                    split_seq_index[1] * tokens_per_seq
                ]
            else:
                raise NotImplementedError
