...
async def health_ready() -> Response:
    """Check whether server initialization and warmup have completed."""
    if (
        _global_state is None
        or not _global_state.ready.is_set()
        or _global_state.tokenizer_manager.gracefully_exit
    ):
    ...
    try:
        if server_args.disaggregation_mode == "null":
            res = requests.post(
                ...
                json=json_data,
                headers=headers,
                timeout=server_args.server_warmup_timeout,
            )
            assert res.status_code == 200, f"{res}"
        elif server_args.disaggregation_mode == "prefill":
            # Only prefill server should do warmup
            ...
            res = requests.post(
                ...
                json=json_data,
                headers=headers,
                timeout=max(1800, server_args.server_warmup_timeout),
            )
            logger.info(
                f"End of prefill warmup with status {res.status_code}, resp: {res.json()}"
            )
        elif server_args.disaggregation_mode == "decode":
            # Decode server warmup: simulate receiving KV cache from prefill server
            logger.info(f"Start of decode warmup ...")
            json_data = {
                "sampling_params": {
                    "temperature": 0.0,
                    "max_new_tokens": 8,
                ...
                "bootstrap_host": [FAKE_BOOTSTRAP_HOST] * server_args.dp_size,
                # This is a hack to ensure fake transfer is enabled during decode warmup
                # ensure each dp rank has a unique bootstrap_room during decode warmup
                "bootstrap_room": [
                    i * (2**63 // server_args.dp_size) + (i % server_args.tp_size)
                    for i in range(server_args.dp_size)
                ],
            ...
            logger.info(
                f"End of decode warmup with status {res.status_code}, resp: {res.json()}"
            )
        else:
            # Unknown disaggregation mode: skip warmup
            logger.info(f"Skipping warmup for {server_args.disaggregation_mode} server")
