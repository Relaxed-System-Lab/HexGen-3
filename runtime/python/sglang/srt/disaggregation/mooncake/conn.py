...
class MooncakeKVManager(BaseKVManager):
    def __init__(
        ...
        # In normal DP mode, server_args.tp_size is per DP group, need global TP size
        # In DP attention mode, server_args.tp_size is already global
        if not server_args.enable_dp_attention:
            self.tp_size = server_args.tp_size * server_args.dp_size
        else:
        ...
        if self.disaggregation_mode == DisaggregationMode.PREFILL:
            ...
            # Counter for tracking chunks being processed
            self.processing_chunks_count = 0
            self.processing_chunks_lock = threading.Lock()
    ...
    def send_kvcache(
        ...
        def process_layer(src_ptr: int, dst_ptr: int, item_len: int) -> int:
            for prefill_index, decode_index in zip(prefill_kv_blocks, dst_kv_blocks):
                ...
                # Log before transfer_sync to track which call causes crash
                try:
                    ...
                    logger.error(
                        f"[Mooncake] Exception in transfer_sync: session_id={mooncake_session_id}, "
                        f"src_addr={src_addr}, dst_addr={dst_addr}, length={length}, error={e}"
                    )
                    raise
                if status != 0:
                    logger.error(
                        f"[Mooncake] transfer_sync returned non-zero status: {status}, "
                        f"session_id={mooncake_session_id}, src_addr={src_addr}, dst_addr={dst_addr}, length={length}"
                    )
                    return status
            return 0
        ...
        # Log executor task count and layer count
        # Note: All futures are just submitted, so they're all pending/running
        # We can check executor's internal queue size if available
        executor_pending = 0
        if hasattr(executor, '_work_queue'):
            executor_pending = executor._work_queue.qsize()
        logger.debug(
            f"[Mooncake] Transfer task submitted to executor: num_layers={num_layers}, "
            f"executor_pending_tasks≈{executor_pending}, session_id={mooncake_session_id}"
        )
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if status != 0:
                failed_count += 1
                logger.error(
                    f"[Mooncake] Layer task failed: completed={completed_count}/{num_layers}, "
                    f"failed={failed_count}, status={status}, session_id={mooncake_session_id}"
                )
                for f in futures:
                    f.cancel()
                return status
            
            # Log progress every 10 tasks or at milestones
            if completed_count < 10 or completed_count % 10 == 0 or completed_count == num_layers:
                logger.debug(
                    f"[Mooncake] Layer tasks progress: {completed_count}/{num_layers} completed, "
                    f"session_id={mooncake_session_id}"
                )

        logger.debug(
            f"[Mooncake] All layer tasks completed: {completed_count}/{num_layers}, "
            f"session_id={mooncake_session_id}"
        )
        return 0
    ...
    def transfer_worker(
        ...
        while True:
            try:                
                ...
                # Get queue size after getting chunk
                with queue._cond:
                    queue_size_after = len(queue._buf)
                
                # Log when chunk leaves transfer_queue and starts processing
                with self.processing_chunks_lock:
                    self.processing_chunks_count += 1
                    processing_count = self.processing_chunks_count
                logger.debug(
                    f"[MoonCake] Chunk left transfer_queue to call executor: room={kv_chunk.room}, "
                    f"queue_size={queue_size_after}, "
                    f"processing_chunks={processing_count}"
                )
                ...
                if (
                    ...
                    if kv_chunk.room in self.transfer_infos:
                        ...
                        # Log resource state after cleanup
                        logger.debug(
                            f"[PREFILL] Resource state after cleanup: "
                            f"transfer_infos_size={len(self.transfer_infos)}, "
                            f"decode_kv_args_table_size={len(self.decode_kv_args_table)}, "
                            f"request_status_size={len(self.request_status)}"
                        )

                # Chunk processing completed, decrease counter
                with self.processing_chunks_lock:
                    self.processing_chunks_count -= 1
                    processing_count = self.processing_chunks_count
                logger.debug(
                    f"[PREFILL] Chunk processing completed: room={kv_chunk.room}, "
                    f"processing_chunks={processing_count}"
                )

            except Exception as e:
                # Decrease counter even on exception
                with self.processing_chunks_lock:
                    self.processing_chunks_count -= 1
    ...
    def add_transfer_request(
        ...
        # Log transfer_queue size when adding a chunk
        with self.transfer_queues[shard_idx]._cond:
            queue_size_before = len(self.transfer_queues[shard_idx]._buf)
        ...
        # Log transfer_queue size after adding, and processing chunks count
        with self.transfer_queues[shard_idx]._cond:
            queue_size_after = len(self.transfer_queues[shard_idx]._buf)
        with self.processing_chunks_lock:
            processing_count = self.processing_chunks_count
        logger.debug(
            f"[PREFILL] Chunk entered transfer_queue: room={bootstrap_room}, "
            f"queue_size={queue_size_after} (was {queue_size_before}), "
            f"processing_chunks={processing_count}"
        )
