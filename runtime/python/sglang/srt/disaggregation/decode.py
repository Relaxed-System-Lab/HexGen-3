...
class DecodePreallocQueue:
    ...
    def _init_kv_manager(self) -> BaseKVManager:
        ...
        # In normal DP mode, tp_size and tp_rank are per DP group
        # In DP attention mode, tp_size is global, tp_rank is global TP rank
        if self.scheduler.server_args.enable_dp_attention:
            # DP attention mode: tp_size is global, attn_tp_size = tp_size // dp_size
            attn_tp_size = self.tp_size // self.dp_size
            # In DP attention mode, tp_rank is global TP rank, but engine_rank should be attn TP rank
            kv_args.engine_rank = self.tp_rank % attn_tp_size
        else:
            # Normal DP mode: tp_size is per DP group, tp_rank is per DP group
            # For bootstrap server routing, engine_rank should be global TP rank
            # Global TP rank = dp_rank * tp_size + tp_rank_in_dp_group
            if self.scheduler.dp_rank is not None:
                # Normal DP mode with dp_size > 1: calculate global TP rank
                kv_args.engine_rank = self.scheduler.dp_rank * self.tp_size + self.tp_rank
            else:
                # dp_size=1: tp_rank is unique, can use as-is
                kv_args.engine_rank = self.tp_rank
            # In normal DP mode, attn_tp_size is same as tp_size (per DP group)
    ...
    def pop_preallocated(self) -> List[DecodeRequest]:
        ...
        # Log when reqs are preallocated
        if preallocated_reqs:
            # Count total preallocated reqs (in transfer_queue)
            total_preallocated = len(self.transfer_queue.queue) + len(preallocated_reqs)
            logger.debug(
                f"[DECODE] {len(preallocated_reqs)} req(s) preallocated KV cache, "
                f"total_preallocated={total_preallocated}"
            )
...
class DecodeTransferQueue:
    ...
    def extend(self, decode_reqs: List[DecodeRequest]) -> None:
        # Log when reqs start waiting for KV cache transfer
        if decode_reqs:
            queue_size_before = len(self.queue)
            self.queue.extend(decode_reqs)
            queue_size_after = len(self.queue)
            logger.debug(
                f"[DECODE] {len(decode_reqs)} req(s) started waiting for KV cache transfer, "
                f"transfer_queue_size={queue_size_after} (was {queue_size_before})"
            )
        else:
            self.queue.extend(decode_reqs)
    ...
    def pop_transferred(self) -> List[Req]:
        ...
        # Log when reqs get KV cache (leave transfer_queue)
        if transferred_reqs:
            queue_size_before = len(self.queue)
            self.queue = [
                entry for i, entry in enumerate(self.queue) if i not in indices_to_remove
            ...
            queue_size_after = len(self.queue)
            logger.debug(
                f"[DECODE] {len(transferred_reqs)} req(s) got KV cache, "
                f"transfer_queue_size={queue_size_after} (was {queue_size_before})"
            )
        else:
            self.queue = [
...
class SchedulerDisaggregationDecodeMixin:
    ...
    @torch.no_grad()
    def event_loop_normal_disagg_decode_afd_attn(self: Scheduler):
        """A scheduler loop for decode(attn) worker in disaggregation mode with AFD.
        
        This combines:
        - PD separation logic: KV cache transfer handling
        - AFD logic: micro-batch support and communication with FFN server
        """
        from sglang.srt.layers.afd import afd_is_attn, get_afd_mirco_batch
        from sglang.srt.managers.io_struct import AFDReqInput
        from sglang.srt.model_executor.forward_batch_info import ForwardMode
        from typing import Sequence
        
        def prepare_overlap(batch: ScheduleBatch):
            """Prepare batch for micro-batch overlap (from event_loop_afd)."""
        ...
        while True:
            # 1. Receive requests and process KV cache transfer (PD separation logic)
            recv_reqs = self.recv_requests()
            self.process_input_requests(recv_reqs)
            # polling and allocating kv cache
            self.process_decode_queue()
            
            # 2. Get batch (PD separation logic)
            batch = self.get_next_disagg_decode_batch_to_run()
            self.cur_batch = batch
            ...
            if batch:
                ...
                if batch.forward_mode.is_extend():
                    ...
                    # 3. Send batch info to FFN server (AFD logic)
                    if afd_is_attn() and self.pp_rank == 0 and self.attn_tp_rank == 0:
                        if self.send_to_ffn is not None:
                            afd_req = AFDReqInput(
                                batch.batch_size(),
                                batch.forward_mode
                            )
                            self.send_to_ffn.send_pyobj(afd_req)
                            logger.debug(f"[AFD] Sent AFDReqInput to FFN: batch_size={batch.batch_size()}, forward_mode={batch.forward_mode}")
                    
                    # 4. Prepare overlap for micro-batch (AFD logic)
                    prepare_overlap(batch)
                    
                    # 5. Prepare MLP sync if needed
                    if prepare_mlp_sync_flag:
                        self.prepare_mlp_sync_batch(batch)
                    
                    # 6. Run batch (only attention, FFN is handled by separate server)
                    result = self.run_batch(batch)
                    self.process_batch_result(batch, result)
            elif prepare_mlp_sync_flag:
                batch, _ = self._prepare_idle_batch_and_run(None)
