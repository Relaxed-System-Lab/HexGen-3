...
class LBArgs:
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--drain-state-file",
            type=str,
            default=LBArgs.drain_state_file,
            help=(
                "JSON state file used by AFD live autoscaler. New requests wait "
                "while state is draining."
            ),
        )
        parser.add_argument(
            "--drain-wait-timeout-s",
            type=float,
            default=LBArgs.drain_wait_timeout_s,
            help="Maximum seconds a new request waits while the runtime is draining.",
        )
...
def main():
    ...
    if lb_args.rust_lb:
        ...
        run(
            ...
            lb_args.host,
            lb_args.port,
            drain_state_file=lb_args.drain_state_file,
            drain_wait_timeout_s=lb_args.drain_wait_timeout_s,
        )
